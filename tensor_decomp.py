...
class SymmetricCPDecomp(object):
    def __init__(self, dim, rank, sess, ndims=3, optimizer_type='2sgd', reg_param=1e-10):
        '''
        `rank` is R, the number of 1D tensors to hold to get an approximation to `X`
    ...
    def train_step(self, approx_indices, approx_values, print_every=10, validate_indices=True):
        if not hasattr(self, 'prev_time'):
            self.prev_time = time.time()
            self.avg_time = 0.0
            self.total_recordings = 0
    ...
    def create_loss_fn(self, reg_param):
        ...
        L_{rho} = L(X; U) + rho * (||U||^2 + ||V||^2 + ||W||^2) where ||.|| represents some norm (L2, L1, Frobenius)
        """
        def L(X, U):
            """
    ...
    def get_update_UVW_ops_for_2sgd(self, rho):
        '''
        See 2SGD algorithms in Expected Tensor Decomp paper
        '''
        def gamma(A,B):
            ATA = tf.matmul(A,A, transpose_a=True)  # A^T * A
            BTB = tf.matmul(B,B, transpose_a=True)  # B^T * B
        ...
        modified_X = contract_X_U(self.X_t, self.U, self.U)
        gamma_rho = gamma(V,W) + rho * tf.eye(self.rank)
        self.gr1 = gamma_rho
        inv_gamma_rho = tf.matrix_inverse(gamma_rho)
        self.gr1inv = gamma_rho
        ...
        return [update_U_op]
        #return [update_U_op, tf.Print(eta_t, [eta_t])]

    def get_train_op_2sgd(self, rho=1e-4):
        update_U_op = self.get_update_UVW_ops_for_2sgd(rho)
        return update_U_op
